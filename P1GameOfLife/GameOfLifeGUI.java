...
public class GameOfLifeGUI extends JFrame {
  ...
  private static Icon icon = new ImageIcon("eclipse.gif");
  ...
  private class ButtonListener implements ActionListener {
    public void actionPerformed(ActionEvent ae) {
      ...
      if (clickedButton.getBackground() == Color.LIGHT_GRAY && frozen) {
  ...
  private void readTheButtons() {
    for (int r = 0; r < society.numberOfRows(); r++)
      for (int c = 0; c < society.numberOfColumns(); c++)
        if (cells[r][c].getBackground() == Color.BLUE)
